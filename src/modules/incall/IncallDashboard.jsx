    ...
    <div>
      ...
      <div className="card card-pad">
        ...
        <div className="table-wrap">
          <table className="tbl">
            <thead><tr>{['유입일자', '엔드유저', '인프라', '담당영업', '진행상태', '수주여부'].map((h) => <th key={h} style={{cursor:'default'}}>{h}</th>)}</tr></thead>
            <tbody>
              {recent.map((r) => (
...
function Kpi({ label, value, color }) {
  return <div className="kpi" style={{ '--accent': color }}><div className="label">{label}</div><div className="value" style={{ color }}>{value}</div></div>;
}
